<script lang="ts">
  ...
  const cartesianPoints = $derived(polarPoints.map(polarToCartesian));
  const frogPoint = $derived(polarPoints[model.position]);

  const message = $derived(model.turnMessage());
...
<Template {model} {board} {config} {rules} {winTitle} />
